...
#+RESULTS:
[[file:./.ob-jupyter/11f1ae0a0d6dc0eb75fb00c233a53553c774f523.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/10fd9177eef486936ab9167884635a34ec83263e.png]]
...
for ax, (prof_num, (label, color)) in zip(axes, virial_profiles.items()):
    ...
    # dm for each shell - flip
    # MESA profiles: index 0 = surface, index -1 = center to reverse for integration
    m_g_rev   = m_g[::-1]
    r_rev     = r_cm[::-1]
    dm        = np.diff(m_g_rev)
    ...
    # Thermal energy: u (erg/g) is specific internal energy
